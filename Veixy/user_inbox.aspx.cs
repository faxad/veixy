...
public partial class staff_inbox : System.Web.UI.Page
{
    ...
    protected void LinkButton7_Click(object sender, EventArgs e)
    {
        ...
        my_update3.ExecuteNonQuery();
        conn.Close();
        SqlCommand my_update1 = new SqlCommand("UPDATE cust_inbox SET lock_by='none' WHERE token_no='" + GridView1.SelectedValue.ToString() + "' AND lock_by='" + Session["user_session"].ToString() + "';", conn);
        conn.Open();
        my_update1.ExecuteNonQuery();
        conn.Close();
        GridView GridVieww = (GridView)this.Master.FindControl("GridView1");
        SqlCommand cmd = new SqlCommand("SELECT loc_sno FROM staff_loc WHERE user_loc_id='" + GridView1.SelectedRow.Cells[2].Text + "' AND loc_type='Ticket';", conn);
        cmd.Connection.Open();
        ...
        dr.Close();
        conn.Close();
        SqlCommand my_delete = new SqlCommand("DELETE FROM staff_loc WHERE loc_sno='" + temp + "' AND loc_by='" + Session["user_session"].ToString() + "';", conn);
        conn.Open();
    ...
    protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (LinkButton1.Text != "Lock Topic")
        {
            DataList1.DataBind();
            LinkButton1.Text = "Lock Topic";
            LinkButton1.ForeColor = System.Drawing.Color.OliveDrab;
    ...
    protected void LinkButton10_Click(object sender, EventArgs e)
    {
        string temp_id;
        temp_id = GridView1.SelectedValue.ToString();
        SqlCommand my_update6 = new SqlCommand("UPDATE cust_inbox SET held_by='none' WHERE token_no='" + temp_id + "';", conn);
        conn.Open();
        my_update6.ExecuteNonQuery();
        conn.Close();
        GridView1.DataBind();
