...
public partial class staff_inbox : System.Web.UI.Page
{
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (RadioButtonList1.SelectedValue.ToString() == "1")
        {
            ...
            SqlCommand my_insert = new SqlCommand("INSERT INTO cust_inbox(token_no,sent_from,msg_subject,img_url,img_url2,start_date,open_close,held_by,association) VALUES(" + x + ",'" + Session["user_session"].ToString() + "','" + TextBox2.Text + "','Images/msg_checked.jpg','Images/msg_checked.jpg','" + my_time + "','O','none','" + temp2 + "');", conn);
            SqlCommand my_insert2 = new SqlCommand("INSERT INTO act_onbox(token_no,act_msg,msg_date,msg_from) VALUES(" + x + ",'" + TextBox3.Text + "','" + my_time + "','" + Session["user_session"].ToString() + "');", conn);
            conn.Open();
        ...
        if (RadioButtonList1.SelectedValue.ToString() == "2")
        {
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                ...
                {
                    continue_ok = 1;
                    SqlCommand my_insert2 = new SqlCommand("INSERT INTO act_onbox(token_no,act_msg,msg_date,msg_from) VALUES(" + token_temp + ",'" + TextBox3.Text + "','" + my_time + "','" + Session["user_session"].ToString() + "');", conn);
                    conn.Open();
                    my_insert2.ExecuteNonQuery();
                    conn.Close();
                    Label1.Visible = true;
