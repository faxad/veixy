...
public partial class staff_inbox : System.Web.UI.Page
{
    ...
    {
        if (GridView1.SelectedIndex.ToString() != "-1")
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    if (dr[0].ToString() == "none")
                    {
                        ...
                        SqlCommand my_insert2 = new SqlCommand("INSERT INTO staff_loc(user_loc_id,loc_by,loc_topic,loc_type) VALUES('" + GridView1.SelectedValue.ToString() + "','" + Session["user_session"].ToString() + "',@topic,'Ticket');", conn);
                        ...
                        Image8.Visible = true;
                        Label1.Visible = true;
                        LinkButton1.Text = "Ticket Locked";
        ...
        {
            System.Web.UI.ScriptManager.RegisterClientScriptBlock(LinkButton4, this.GetType(), "pop up", "alert('Please select a row first.');", true);
        }
    }
    ...
    protected void LinkButton7_Click(object sender, EventArgs e)
    {
        ...
        SqlCommand my_insert2 = new SqlCommand("INSERT INTO act_onbox(token_no,act_msg,msg_date,msg_from) VALUES('" + Session["temp"].ToString() + "',@msg,'" + my_time + "','" + Session["user_session"].ToString() + "');", conn);
        my_insert2.Parameters.Add("@msg", 8000).Value = TextBox3.Text;
        SqlCommand my_update3 = new SqlCommand("UPDATE cust_inbox SET img_url='Images/msg_unchecked.jpg',img_url2='Images/msg_replied2.jpg' WHERE token_no='" + Session["temp"].ToString() + "';", conn);
        conn.Open();
        my_insert2.ExecuteNonQuery();
        my_update3.ExecuteNonQuery();
        conn.Close();
        SqlCommand my_update1 = new SqlCommand("UPDATE cust_inbox SET lock_by='none' WHERE token_no='" + Session["temp"].ToString() + "';", conn);
        conn.Open();
        ...
        GridViewww.DataBind();
        fetch_user_info();
        LinkButton1.Text = "Lock Topic";
        LinkButton1.ForeColor = System.Drawing.Color.OliveDrab;
        if (cust_profile[1] == '1')
        {
            email_class obje = new email_class();
    ...
    protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (LinkButton1.Text != "Lock Ticket")
        {
            DataList1.DataBind();
            LinkButton1.Text = "Lock Ticket";
            LinkButton1.ForeColor = System.Drawing.Color.OliveDrab;
            TextBox3.Text = "";
            TextBox3.Enabled = false;
            TextBox3.Text = "Please lock the ticket in order to reply.";
            GridView1.DataBind();
