...
public partial class staff_inbox : System.Web.UI.Page
{
        ...
        LinkButton LinkButton1 = (LinkButton)dli.FindControl("LinkButton1");
        {
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                if (dr[0].ToString() == "none")
                {
                    ...
                    SqlCommand my_insert2 = new SqlCommand("INSERT INTO staff_loc(user_loc_id,loc_by,loc_topic,loc_type) VALUES('" + GridView1.SelectedValue.ToString() + "','" + Session["user_session"].ToString() + "',@subject,'Call');", conn);
    ...
    protected void LinkButton7_Click(object sender, EventArgs e)
    {
        ...
        SqlCommand my_update1 = new SqlCommand("UPDATE my_calls SET lock_by='none' WHERE call_ref_no='" + Session["temp"].ToString() + "';", conn);
        conn.Open();
        my_update1.ExecuteNonQuery();
        conn.Close();
        GridView GridVieww = (GridView)this.Master.FindControl("GridView1");
        SqlCommand my_delete = new SqlCommand("DELETE FROM staff_loc WHERE loc_sno='" + GridVieww.DataKeys[(GridVieww.Rows.Count) - 1].Value.ToString() + "' AND loc_type='CALL';", conn);
        conn.Open();
        ...
        LinkButton LinkButton1 = (LinkButton)DataList1.Items[0].FindControl("LinkButton1");
        LinkButton1.Text = "Call Locked";
        LinkButton1.ForeColor = System.Drawing.Color.DarkRed;
        LinkButton LinkButton2 = (LinkButton)DataList1.Items[0].FindControl("LinkButton2");
        LinkButton LinkButton7 = (LinkButton)DataList1.Items[0].FindControl("LinkButton7");
